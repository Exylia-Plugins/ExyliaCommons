...
public abstract class SQLAdapter implements DatabaseAdapter {
    ...
    public void connect() throws Exception {
        try {
            ...
            ds.setIdleTimeout(config.getIdleTimeoutMs());
            ds.setMaxLifetime(config.getMaxLifetimeMs());
            ds.setAutoCommit(true);
            this.dataSource = ds;
            DebugUtils.logInfo("Connected to " + getAdapterName());
        } catch (Exception e) {
            DebugUtils.logError("Failed to connect to database: " + e.getMessage());
            throw e;
        }
    }
    ...
    public void disconnect() throws Exception {
        if (dataSource != null && !dataSource.isClosed()) {
            dataSource.close();
            DebugUtils.logInfo("Disconnected from " + getAdapterName());
        }
    }
    ...
    public void createTable(EntityMetadata metadata) throws Exception {
        if (!tableExists(metadata)) {
            ...
            try (Connection conn = dataSource.getConnection();
                ...
                stmt.execute(sql);
                DebugUtils.logInfo("Created table: " + metadata.getTableName());
            }
        }
    }

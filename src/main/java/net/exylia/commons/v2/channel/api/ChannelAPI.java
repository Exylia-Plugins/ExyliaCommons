...
public final class ChannelAPI {
    ...
    public static void create(String id, String permission, String format, double cooldownSeconds) {
        validateId(id);
        ...
        if (cooldownSeconds > 0) {
            ChannelManager.getInstance().getCooldownManager().registerChannel(id, cooldownSeconds);
        }

        ChannelMessenger messenger = ChannelManager.getInstance().getMessenger();
    ...
    public static void delete(String channelId) {
        ...
        ChannelManager.getInstance().getWriteModeTracker().clearAllForChannel(channelId);
        ChannelManager.getInstance().getCooldownManager().unregisterChannel(channelId);
    }

    public static void updateFormat(String channelId, String newFormat) {
        validateFormat(newFormat);

        Channel oldChannel = get(channelId)
    ...
    public static void setCooldownMessage(String channelId, String cooldownMessage) {
    ...
    public static void setWriteMode(Player player, String channelId) {
        ...
        WriteModeTracker tracker = ChannelManager.getInstance().getWriteModeTracker();
    ...
    public static void sendMessage(String channelId, Player sender, String message) {
        if (sender == null || message == null) {
            throw new IllegalArgumentException("Sender and message cannot be null");
        }

        ChannelManager.getInstance().getMessenger().sendMessage(channelId, sender, message);
    }
    ...
    public static void broadcast(String channelId, String message) {
        ...
        PermissionCache cache = ChannelManager.getInstance().getPermissionCache();
    ...
    private static void validateId(String id) {
        ...
        if (!id.matches("[a-z0-9-]+")) {
            throw new IllegalArgumentException("Channel ID can only contain lowercase letters, numbers, and hyphens");
        }
    }

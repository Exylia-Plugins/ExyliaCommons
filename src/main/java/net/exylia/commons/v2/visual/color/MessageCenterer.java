...
public class MessageCenterer {
    private static final int CENTER_PX = 154;
    private static final int MAX_PX = 320;
    ...
    public static String center(String message) {
        ...
        String processedMessage = ColorProcessor.processString(message.replace('§', '&'));

        int messagePxSize = calculatePixelWidth(processedMessage);
    ...
    public static List<String> centerMultiple(List<String> messages) {
        ...
        for (String message : messages) {
            centered.add(center(message));
        }
        return centered;
    }

    public static String centerWithMaxWidth(String message, int maxWidth) {
        if (message == null || message.isEmpty()) {
            return message;
        }
        ...
        if (messagePxSize > maxWidth) {
            return message;
        }

        int halvedMessageSize = messagePxSize / 2;
        int centerPoint = maxWidth / 2;
        int toCompensate = centerPoint - halvedMessageSize;
        int spaceLength = DefaultFontInfo.SPACE.getLength() + 1;
        int compensated = 0;
    ...
    public static int getPixelWidth(String message) {
        ...
        String processedMessage = ColorProcessor.processString(message.replace('§', '&'));
        return calculatePixelWidth(processedMessage);
    }

    private static int calculatePixelWidth(String processedMessage) {
        int messagePxSize = 0;

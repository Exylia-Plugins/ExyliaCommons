...
If `redis.enabled: false`, `init(plugin)` returns without creating an instance. Check
`SimpleRedis.isInitialized()` before calling `get()`/`getInstance()`. Connection/PING failures can
throw during initialization. `init(...)` throws `IllegalStateException("SimpleRedis already
initialized")` if called twice. `reload(plugin)` has separate first-call/reload behavior and does
not make an existing disabled instance disappear.
...
static SimpleRedis get();          // preferred accessor
static SimpleRedis getInstance();  // also generated by Lombok and used internally
...
// Async command (offloaded; uses the JVM common pool)
SimpleRedis.get().executeAsync(jedis -> jedis.get("key")).thenAccept(v -> { /* handle */ });

// Publish / subscribe. Raw commands and channel names do not receive key-prefix automatically;
// namespace them explicitly when needed.
SimpleRedis.get().pubSub().publish("channel", "message");

// Subscribers must be registered and explicitly unsubscribed by the owning plugin.
SimpleRedis.get().pubSub().subscribeObject("channel", Announcement.class,
    announcement -> Tasks.sync(() -> handleAnnouncement(announcement)));
// SimpleRedis.get().pubSub().unsubscribe("channel");
// SimpleRedis.get().pubSub().unsubscribeAll();
```
...
- `executeAsync` uses the JVM common pool; `publishAsync` uses SimpleRedis's dedicated async
  executor. Neither is a Bukkit thread.
...
- Check **both** `isInitialized()` and `isConnected()` before relying on Redis-dependent features.
- Call `SimpleRedis.shutdown()` from your plugin's disable hook. The shared shutdown coordinator
  does not shut down standalone SimpleRedis automatically.

## Common Mistakes

- Calling `get()` when `redis.enabled: false` — no instance is created; check `isInitialized()`.
...
- Calling `init(...)` twice → `IllegalStateException`, or assuming a failed PING is converted to a
  false result.
- Forgetting that Jedis is not bundled by ExyliaCommons.
- Reloading Redis while `ChannelAPI` is active: the channel messenger retains the old Redis instance
  and has no public rebind operation. Reinitialize channels/messenger as a coordinated operation.
